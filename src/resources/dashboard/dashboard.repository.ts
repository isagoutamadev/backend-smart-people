    ...
    async getReservationRealizationGraph(search: SearchDasboardReservationRealization): Promise<DashboardReservationRealizationGraph[]> {
        try {
            const select: any[] = [
                ...
                knex.raw("sum(reservation.count) as total_count_reservation"),
            ];
            ...
            query.groupByRaw("datetime");
            query.orderByRaw("datetime asc");

            if (search.group_time === 'monthly') {
                select.push(knex.raw("CONVERT_TZ(CONCAT(YEAR(reservation_date), '-', MONTH(reservation_date), '-15 17:00:00'),'UTC','UTC') as datetime"));
            } else {
                select.push("reservation_date as datetime");
            }
