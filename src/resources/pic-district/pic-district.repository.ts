...
export class PicDistrictRepository {
    async get(search: PicDistrict, page: number, limit: number): Promise<Paging<PicDistrict>> {
        try {
            const select = [
                "id",
            ];
            ...
            const pagination = new Pagination<PicDistrict>(
                datas,
                //@ts-ignore
                count.total,
                page,
