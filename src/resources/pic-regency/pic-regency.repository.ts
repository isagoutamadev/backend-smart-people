...
export class PicRegencyRepository {
    async get(search: PicRegency, page: number, limit: number): Promise<Paging<PicRegency>> {
        try {
            const select = [
                "id",
            ];
            ...
            const pagination = new Pagination<PicRegency>(
                datas,
                //@ts-ignore
                count.total,
                page,
