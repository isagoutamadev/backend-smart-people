...
export class OrganizationRegencyRepository {
    async get(search: OrganizationRegency, page: number, limit: number): Promise<Paging<OrganizationRegency>> {
        try {
            const select = [
                "id",
            ];
            ...
            const pagination = new Pagination<OrganizationRegency>(
                datas,
                //@ts-ignore
                count.total,
                page,
